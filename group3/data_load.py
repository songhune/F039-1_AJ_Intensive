...
def load_data(mode="train"):
    ...
    if mode=="train":
        ...
        transcript = os.path.join(hp.data, 'transcript.v.1.1.txt').replace("\\", "/")
...
def new_load_data(ko_txt, mode="train"):
    '''Loads data
      Args:
          mode: "train" or "synthesize".
    '''
    ...
    if mode=="train":
        ...
        lines = ko_txt.splitlines()
        sents = [_normalize(line) for line in lines[:]]
        texts = np.zeros((len(sents), hp.max_N), np.int32)
        for i, sent in enumerate(sents):
            texts[i, :len(sent)] = sent
        return texts
...
def get_batch():
    ...
    with tf.device('/cpu:0'):
        ...
        def _load_spectrograms(fpath):
            ...
            mel = (os.getcwd() + "/mels/{}").replace("\\", "/").format(fname.replace("wav", "npy"))
            mag = (os.getcwd() + "/mags/{}").replace("\\", "/").format(fname.replace("wav", "npy"))
